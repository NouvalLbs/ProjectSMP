...
namespace ProjectSMP.Entities.Vehicles.Seatbelt
{
    public static class SeatbeltService
    {
        ...
        public static void OnPlayerStateChanged(Player player, PlayerState newState)
        {
            ...
            if (auto)
                player.SendClientMessage(-1, $"{Msg.Vehicles} Seatbelt {{00FF00}}ON{{FFFFFF}} (Otomatis)");
            else
                player.SendClientMessage(-1, $"{Msg.Vehicles} Seatbelt {{FF0000}}OFF{{FFFFFF}} - ketik {{FFFF00}}/seatbelt{{FFFFFF}} untuk memasangnya.");
        }
        ...
        private static void OnVehicleImpacted(object? sender, VehicleImpactArgs e)
        {
            ...
            var drunkLevel = Math.Clamp((int)(e.Force * 12000), 1500, 6000);
